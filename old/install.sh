...
echo -e "\e[91mIPv6\e[39m temporär bis zum neustart deaktivieren (y/n)?"
echo -e "disable \e[91mIPv6\e[39m until reboot (y/n)?"
read answer
if echo "$answer" | grep -iq "^y" ;
then
  echo 1 > /proc/sys/net/ipv6/conf/all/disable_ipv6
else
  echo not blocking IPv6
fi

apt-get install sudo screen vim
...
echo -e "Install \e[91mMulti-Factor authentication\e[39m \e[33mdafür bitte einen leeren memorystick eintecken\e[39m und z.B. 4 partitionen erstellen (y/n)?"
read answer
if echo "$answer" | grep -iq "^y" ;then
